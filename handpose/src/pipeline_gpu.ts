...
export class HandPipeline {
  ...
  private generateCaseInputs(totalSizeTensor: number) {
    ...
    for (let i = 0; i < totalSizeTensor; i++) {
      inp[i] = i * 0.0000098460388184 + 0.1571898460388184;
    }
    return inp;
  }
  ...
  async estimateHand(image: tf.Tensor4D): Promise<Prediction> {
    ...
    console.log("palmCenterNormalized="+palmCenterNormalized);
...
//-----------
{
  ...
  const inputShape: [number, number, number, number] =
      [1, xSize, ySize, inputDepth];
  const inputs = this.generateCaseInputs(1 * xSize * ySize * inputDepth);
  const imagex = tf.tensor4d(inputs, inputShape);
...
//-----------
    ...
    console.warn("handImage="+ await handImage.data());
    ...
    */
    ...
    if (0)
    return null;
    const flag: tf.Tensor = null;
    const keypoints: tf.Tensor = null;
    */
